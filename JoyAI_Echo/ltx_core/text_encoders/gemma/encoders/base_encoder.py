...
def load_gemma(module,gemma_root,clip_path):
    from transformers import Gemma3ForConditionalGeneration, Gemma3Config
    ctx = init_empty_weights if is_accelerate_available() else nullcontext
    
    configuration = Gemma3Config.from_pretrained(gemma_root)

    with ctx():
        meta_model=Gemma3ForConditionalGeneration(configuration)
    ...
    model_state_dict=load_gguf_checkpoint_gemma(clip_path) 
    ...
    if embed_tokens_key:
        adjusted_state_dict['lm_head.weight'] = adjusted_state_dict[embed_tokens_key]
