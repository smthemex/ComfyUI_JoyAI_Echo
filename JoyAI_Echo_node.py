...
class JoyAI_Echo_SM_KSampler(io.ComfyNode):
    ...
    @classmethod
    def execute(cls, model,width,height,seed,num_frames,frame_rate,prefetch_count,te_cond=None) -> io.NodeOutput:
        clear_comfyui_cache()
        if te_cond is None:
...
class JoyAI_Echo_SM_Clip(io.ComfyNode):
    ...
    def execute(cls,clip,connector,infer_device ) -> io.NodeOutput:
        ...
        clip=load_joyai_te(gemma_path,connector_path,gemma_root,torch.device(infer_device))
...
class JoyAI_Echo_SM_Encoder(io.ComfyNode):
    ...
    @classmethod
    def execute(cls,clip,prefetch_count,prompt,prompt_files) -> io.NodeOutput:
        clear_comfyui_cache()
        ...
        te_cond=infer_joyai_text(clip,prompt_files)
